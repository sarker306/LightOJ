...
void printMatrix( int array[SIZE][SIZE], int n ){
    ...
    puts("");
    for ( i = 1 ; i <= n ; i++ ){
        for ( j = 1 ; j <= n ; j++ ) printf("%3d", array[i][j] );
        puts("");
    }
    puts("");
}
...
int ProcessMaxScore(long *score1, long *score2, int n){
    int i, j;
    int array[SIZE][SIZE], matching[SIZE], totalScore = 0;
    
    for ( i = 1 ; i <= n ; i++ )
        for ( j = 1 ; j <= n ; j++ )
            if ( score1[i] > score2[j] ) array[i][j] = -2;
            else if ( score1[i] < score2[j] ) array[i][j] = 0;
            else array[i][j] = -1;
    ...
    for ( i = 1 ; i <= n ; i++ )
        if ( score1[i] > score2[matching[i]] ) totalScore += 2;
        else if ( score1[i] == score2[matching[i]] ) totalScore++;
...
int main(){
    int test, kase, n, i;
    long score1[SIZE], score2[SIZE];
    
    while ( scanf("%d", &test) == 1 ){
        for ( kase = 1 ; kase <= test ; kase++ ){
            scanf("%d", &n);
            for ( i = 1 ; i <= n ; i++ ) scanf("%ld", &score1[i]);
            for ( i = 1 ; i <= n ; i++ ) scanf("%ld", &score2[i]);
            printf("Case %d: %d\n", kase, ProcessMaxScore(score1, score2, n));
        }
    }
